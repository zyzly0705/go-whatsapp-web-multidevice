export default {
        ...
        async saveCurrentRule() {
            ...
            try {
                const response = await window.http.post('/dingtalk/config', {
                    ...
                    rules: this.rules.map(item => this.ruleToPayload(item)),
                });
    ...
    <div class="alert-workbench">
        ...
        <div class="workbench-grid main-workspace-grid">
            <section class="tool-panel">
                ...
                <div class="account-status" :class="{loading: loadingDevices}">
                    ...
                    <span>{{ loginStateDescription }}</span>
                </div>
            ...
            <section class="tool-panel rule-list-panel">
                <div class="section-title-row">
                    <h3><i class="sliders horizontal icon"></i> 转发规则</h3>
                    <button class="ui button" @click="addRule">新增规则</button>
                </div>
                <div class="config-file-card">
                    <strong>本地配置文件</strong>
                    <span>{{ configPath || '尚未生成，首次保存后创建' }}</span>
                </div>
                <div class="ui toggle checkbox global-toggle">
                    <input type="checkbox" v-model="configEnabled">
                    <label>{{ configEnabled ? '总开关已启用' : '总开关未启用' }}</label>
                </div>
                <div class="rules-list">
                    <button v-for="rule in rules" :key="rule.id"
                            class="rule-list-item"
                            :class="{selected: rule.id === selectedRuleId, disabled: !rule.enabled}"
                ...
                <div class="notice-panel" v-if="lastSavedSummary">
                    <strong>保存回显</strong>
        ...
        <section class="tool-panel" v-if="selectedRule">
            <div class="section-title-row">
                <h3><i class="filter icon"></i> 编辑当前规则：{{ selectedRule.name }}</h3>
                <button class="ui red button" :disabled="rules.length <= 1" @click="removeSelectedRule">删除当前规则</button>
            </div>
            <div class="rule-editor-grid">
                <div class="ui form">
                    <div class="two fields">
                        <div class="field">
                            <label>规则名称</label>
                            <input v-model="selectedRule.name" placeholder="例如：财务群付款提醒">
                        </div>
                        <div class="field">
                            <label>规则状态</label>
                    ...
                    <div class="field">
                        <label>关键词</label>
                        <input v-model="selectedRule.keywordsText" placeholder="留空 = 所有消息；多个用英文逗号分隔">
                        <small>当前：{{ keywordLabel }}</small>
                    </div>
                    <div class="field">
                        <label>通知时段</label>
                        <div class="time-range-row">
                            <select v-model="selectedRule.timeStart" aria-label="通知开始时间">
                                <option v-for="time in timeOptions" :key="'start-' + time" :value="time">{{ time }}</option>
                            </select>
                            <span>至</span>
                            <select v-model="selectedRule.timeEnd" aria-label="通知结束时间">
                    ...
                    <div class="field">
                        <label>个人消息</label>
                        <div class="ui toggle checkbox">
                            <input type="checkbox" v-model="selectedRule.monitorDirect">
                            <label>{{ selectedRule.monitorDirect ? '群消息和个人消息都监控' : '只监控群消息' }}</label>
                        </div>
                        <small>群选择只限制群消息；个人消息只按关键词和通知时段判断。</small>
                    </div>
                    <div class="two fields">
                        <div class="field">
                            <label>通知标题</label>
                            <input v-model="selectedRule.title">
                        </div>
                        <div class="field">
                            <label>消息最大长度</label>
                            <input type="number" min="1" v-model.number="selectedRule.maxBodyLength">
                        </div>
                    </div>
                </div>
                ...
                <div class="ui form">
                    <div class="help-panel">
                        ...
                        <span>安全设置建议选择“加签”，复制 Webhook，并把 SEC 开头的加签密钥填到这里。</span>
                    </div>
                    <div class="field">
                        <label>机器人别名</label>
                        <input v-model="selectedRule.webhookAlias" placeholder="例如：财务预警机器人">
                        <small>用于页面和历史记录展示，会写入本地配置。</small>
                    </div>
                    <div class="field">
                        <label>Webhook</label>
                        <input type="password" v-model="selectedRule.webhook" :placeholder="selectedRule.webhookConfigured ? '已保存，留空表示不修改' : '粘贴钉钉机器人 Webhook'">
                        <small class="config-status" :class="{ok: selectedRule.webhookConfigured}">{{ selectedWebhookStatusLabel }}；输入新地址才会覆盖。</small>
                    </div>
                    <div class="field">
                        <label>加签密钥</label>
                        <input type="password" v-model="selectedRule.secret" :placeholder="selectedRule.secretConfigured ? '已保存，留空表示不修改' : '粘贴 SEC 开头的加签密钥'">
                        <small class="config-status" :class="{ok: selectedRule.secretConfigured}">{{ selectedSecretStatusLabel }}；不会在页面回显明文。</small>
                    </div>
                    <div class="field">
                        <label>At 手机号</label>
                        <input v-model="selectedRule.atMobilesText" placeholder="可选，多个用英文逗号分隔">
                    </div>
                    <div class="field">
                        <div class="ui toggle checkbox">
                            <input type="checkbox" v-model="selectedRule.atAll">
                            <label>钉钉消息 @ 所有人</label>
                        </div>
                    </div>
                    <div class="button-row">
                        <button class="ui primary button" :class="{loading: savingCurrent}" @click="saveCurrentRule">校验并保存当前规则</button>
                        <button class="ui button" :class="{loading: testing}" @click="testCurrentRule">只测试当前规则</button>
                    </div>
                    <small>保存前会先发送一条钉钉测试消息；测试失败不会写入配置文件。</small>
                </div>
            </div>
        </section>
        ...
        <section class="tool-panel">
            <div class="section-title-row">
                <h3><i class="users icon"></i> 当前规则的群监控范围</h3>
            ...
            <div class="summary-line">{{ selectedRule?.name || '当前规则' }}：{{ monitorScopeLabel }}</div>
            ...
            <div class="forward-history-list" :class="{loading: loadingHistory}">
                <div class="forward-history-row" v-for="item in forwardHistory" :key="item.id">
                    ...
                    <div class="forward-history-meta">
                        ...
                        <span><strong>规则</strong>{{ item.rule_name || '-' }}</span>
                        <span><strong>机器人</strong>{{ item.robot_alias || '-' }}</span>
